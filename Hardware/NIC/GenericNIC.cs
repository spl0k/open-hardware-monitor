...
namespace OpenHardwareMonitor.Hardware.NIC
{
    internal class GenericNIC : Hardware
    {
            ...
            : base(name, new Identifier("nic", index.ToString(CultureInfo.InvariantCulture)), settings)
        {
            ...
            sensors[0] = new Sensor("Bytes recieved per sec", 0, SensorType.Data, this, settings);
            sensors[1] = new Sensor("Bytes sent per sec", 1, SensorType.Data, this, settings);
            sensors[2] = new Sensor("Total bytes per sec", 2, SensorType.Data, this, settings);
